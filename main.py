...
def main():
    ...
    parser.add_argument('--model_name', type=str, default='ddpm_conditional_mnist',
                        choices=['ddpm_conditional_mnist', 'ddpm'])
    parser.add_argument('--pretrained_weights', type=str, default='/content/model_39.pth')  # path to pretrained weights
    parser.add_argument('--path_noise', type=str,
                        default='/content/content/noise')  # paths to dataset of noise-img pairs
    parser.add_argument('--path_images', type=str,
                        default='/content/content/images')  # paths to dataset of noise-img pairs
    ...
    parser.add_argument('--T', type=float, default=400)  # 400 for ddpm_conditional_mnist, 1000 for unconditional
    ...
    parser.add_argument('--save_dir', type=str,
                        default='/content/drive/MyDrive/Colab Notebooks/diff_dist_cifar.pth')  # where to save checkpoint
    ...
    if args.model_name == 'ddpm_conditional_mnist':
        ...
        mu_real = ConditionalDDPM(ContextUnet(1, n_feat=128), betas=(args.sigma_min, args.sigma_max), n_T=args.T,
                                  device=device, drop_prob=args.drop_prob)
        mu_real.load_state_dict(checkpoint)
        mu_fake = ConditionalDDPM(ContextUnet(1, n_feat=128), betas=(args.sigma_min, args.sigma_max), n_T=args.T,
                                  device=device, drop_prob=args.drop_prob)
        mu_fake.load_state_dict(checkpoint)
        netG = ConditionalDDPM(ContextUnet(1, n_feat=128), betas=(args.sigma_min, args.sigma_max), n_T=args.T,
                               device=device, drop_prob=args.drop_prob)
        netG.load_state_dict(checkpoint)
    elif args.model_name == 'ddpm':
        checkpoint = torch.load(args.pretrained_weights, map_location=device)
        mu_real = DDPM(UNet(args.n_T), n_steps=args.n_T, min_beta=args.sigma_min, max_beta=args.sigma_max,
                       device=device)
        mu_real.load_state_dict(checkpoint)
        forward_diffusion = mu_real.forward
        mu_fake = DDPM(UNet(args.n_T), n_steps=args.n_T, min_beta=args.sigma_min, max_beta=args.sigma_max,
                       device=device)
        mu_fake.load_state_dict(checkpoint)
        netG = DDPM(UNet(args.n_T), n_steps=args.n_T, min_beta=args.sigma_min, max_beta=args.sigma_max, device=device)
        netG.load_state_dict(checkpoint)
    else:
