...
class DDPM(nn.Module):
    ...
    def sample(self, n_samples, size, x=None, guide_w=None):
        ...
        c, h, w = size
        if x is None:
            x = torch.randn([n_samples, c, h, w]).to(self.device)  # N(0,1) ([n_samples, c, h, w])

        for idx, t in enumerate(range(self.n_T - 1, -1, -1)):
            time_tensor = (torch.tensor([t] * n_samples).reshape(-1, 1).long().to(self.device))
            eta_theta = self.backward(x, time_tensor.reshape(-1))
            ...
            if t > 0:
                ...
                sigma_t = ((1 - alpha_t_m1_bar) / (1 - alpha_t_bar) * beta_t) ** 0.5
                x = x + sigma_t * z
        return x
